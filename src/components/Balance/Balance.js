...
const Balance = ({ total, difference, symbol /* , account_name */ }) => (
  <div className="-is-logged-in">
    <div className={cx("financials u-p3", css.financials)}>
      ...
      <p className={cx("full_balance subtitle is-6", css.full_balance)}>{numeral(total).format('0,0[.]0000')}</p>
      {difference && <div className={cx("tag is-primary change", css.tag)}>{symbol}{difference}</div>}
    </div>
  </div>
);
